...
ColBasis::usage=
"ColBasis[A] returns a basis for the column space of the general matrix A."
...
adjoint::usage=
  "adjoint[xi] gives the adjoint matrix corresponding to the Lie algebra element xi."
...
ObjectInertia::usage=
"ObjectInertia[fstring, pars, Mass, InertiaTensor] returns the matrix B(xe) for the supplied: fstring (string that specifies the chosen SO(3) parametrization, e.g., 'EulZYZ'),
pars (6-vector with x and Phi coordinates), Mass (object mass), InertiaTensor (object inertia tensor)."

InertiaToCoriolis::usage=
"InertiaToCoriolis[B(q), q, qp] computes the Coriolis matrix given: the inertia matrix B, the joint variables q, joint variable derivatives qp."

InertiaToCoriolisNotChristoffel::usage=
"InertiaToCoriolisNotChristoffel[B(q), q, qp] computes the Coriolis matrix without the property required for skew-symmetry of N = Bp - 2C: the inertia matrix B, the joint variables q,
...
ObjectGravitational::usage=
"ObjectGravitational[fstring, pars, Mass, g0] returns the gravitational vector for the supplied: fstring (string that specifies the chosen SO(3) parametrization, e.g., 'EulZYZ'),
pars (6-vector with x and Phi coordinates), Mass (object mass), g0 (gravitational acceleration components)."
...
ObjectDynamicEquations::usage=
"ObjectDynamicEquations[fstring, Mass, InertiaTensor, gb, x, xp, v, vp] computes the LHS B(x)vp + C(x,xp)v + G(x) for the supplied:
fstring (string that specifies the chosen SO(3) parametrization, e.g., 'EulZYZ'), Mass (object mass), InertiaTensor (object inertia tensor),
gb (componenents of gravity in {B}), x (config.), xp (velocity), v (Slotine-Li velocity), vp (Slotine-Li acceleration)."
...
"ConstraintMatrix[type, axis, Rbe] returns the constraint matrix associated with the constraint type, its axis (if applicable) and Sb to Se transformation Rbe. Allowed contraint types are:\n\
...
ColBasis[A_] :=
	Module[
		...
		RREFtA = RowReduce[tA, Tolerance->Automatic];
		
		basis = Select[RREFtA, Norm[#] > 0 &] // Transpose;
...
ObjectInertia[fstring_, pars_, Mass_, InertiaTensor_]:=
	Module[
			{Bmat, fBodyJac,
		     Jv, Jo, JvT, JoT},
			...
			fBodyJac = ToExpression[fstring <> "ToBodyJac"];
		        
		    (* attenzione: l'origine della terna EE coincide con il baricentro dell'oggetto *)
		    (* in pars ci vanno {x, Phi}, coords cartesiane x dell'origine, angoli Phi nella parametrizzazione scelta *)
		    
		    Jv =  Eye[3];
		    Jo = fBodyJac@@pars[[4;;6]];  
		    
		    JvT = Transpose[Jv];
		    JoT = Transpose[Jo];
		    
		    Bmat = BlockDiag[{Mass JvT.Jv, JoT.InertiaTensor.Jo}]; 
...
ObjectGravitational[fstring_, pars_, Mass_, g0_]:=
	Module[{Gvec, Jv, JvT},
	
	Gvec = (ZeroMatrix[1, 6])[[1]];
	
	Jv  = StackCols[ Eye[3], ZeroMatrix[3] ];
	JvT = Transpose[Jv];
	Gvec = -Mass JvT.g0; 
...
ObjectDynamicEquations[fstring_, Mass_, InertiaTensor_, gb_, x_, xp_, v_, vp_]:=
	Module[{B, C, G, eqs},
		
		B = ObjectInertia[fstring, x, Mass, InertiaTensor];
		C = InertiaToCoriolis[B, x, xp];
		G = ObjectGravitational[fstring, x, Mass, gb];
