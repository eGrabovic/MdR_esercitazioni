classdef kinematicTree < handle
    ...
    methods
        ...
        function [qdd, Cbar] = ABAdynCasadi(this, q, qd, tau, V0, V0d, Fext, options)
            %
            % q = istantaneous joint configuration
            % qd = istantaneous joint velocity
            % V0 = istantaneous base velocity
            ...
            for i = this.Nb:-1:1
                ...
                for j = 1:length(m)
                    ...
                    btilde(:,i) =  btilde(:,i) + ADstar*btilde(:,children) - ADstar*Mbar{children}*a{children}*AdGinv{children}*twists(:, i+1)+...
                        (1./(mscalar(children)))*ADstar*Mtilde{children}*X(:,children)*(tau(children) - X(:,children).'*btilde(:, children));
                end
                
                mscalar(i) = (X(:,i)).'*Mtilde{i}*X(:,i);
                
                Mbar{i} = (eye(6) - Mtilde{i}*X(:,i)*X(:,i).'./(mscalar(i)))*Mtilde{i};
                if ~isempty(C)
                    Cbar(i, :) = -C(i, :);
                    if isempty(m)
                        for kk = 1:nc
                    ...
                    for j = 1:length(m)
                        children = m(j);
                        ADstar = AdGinv{children}.';
                        AA = -ADstar* Mtilde{children}*X(:,children)./mscalar(children);
                        BB = ADstar*(E.*mscalar(children) - Mtilde{children}*X(:,children)*X(:,children)')./mscalar(children);
                        for kk = 1:nc % loop on multipliers
                            L{i, kk} = (AA.*C(children, kk))  + ...
                                BB*L{children, kk};
                            Cbar(i, kk) = Cbar(i, kk) - X(:,i).'*L{i, kk};
                        end
                    end
                    Cbar(i, :) = Cbar(i, :)./mscalar(i);
                end
            end
            ...
            for i = 1:this.Nb
                ...
                Vlambda = Aji*twists(:,this.lambda(i)+1); % i+1-th twist actually refers to the i-th body
                VlambdaD = Aji*twistsD(:,this.lambda(i)+1);
                qdd(i) = (tau(i) - X(:,i).'*(Mtilde{i}*(VlambdaD - a{i}*Vlambda) + btilde(:,i)))./mscalar(i);
                twistsD(:, i+1) = VlambdaD + X(:,i).*qdd(i) - a{i}*Vlambda;
            end
        end
        ...
        function computeCasadi_state_space_dyn(this, options)
            ...
            arguments
                this
                options.constraint = []
                options.error = []; % error might be useful to implement Baumgarte constraint stabilization
            end
            ...
            if ~isempty(options.constraint)
                ...
                C2 = Cbar*C1;
                Cdq = jacobian(C,q);
                Cdot = zeros(size(Cdq(:,1)));
                for ii = 1:length(qd)
                   Cdot = Cdot + Cdq(:, ii).*qd(ii); 
                end
                Cdot = reshape(Cdot, size(C));
                qdd = (eye(this.Nb) - C2*C)*ABA - C2*(Cdot*qd);
                lam = C1*C*ABA - C1*Cdot*qd;
            end
            xdot = [qd; qdd];
        ...
        function compileDyn(this)
            ...
            if isempty(this.xdot_sym)
               error('No dynamic equations found. Build the casadi function for the dynamic equations first!') 
            end
